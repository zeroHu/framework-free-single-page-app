...
notfound.init = function() {
    alert('url is not exist , please check ');
}
...
miniSPA.render = function(url) {
    ...
    console.log('======render',url,settings.rootScope);
...
miniSPA.changeUrl = function() {
    ...
    console.log('changeUrl====url', url, window[url].partial);
...
miniSPA.ajaxRequest = function(url, method, data, callback) {
    ...
    console.log('ajaxRequest======settings', settings.partialCache);
    if (settings.partialCache[url]) {
        console.log('ajaxRequest======== go back has partialCache');
                ...
                if (xmlhttp.readyState == 4) {
                    switch (xmlhttp.status) {
                        case 404: //if the url is invalid, show the 404 page
                            ...
                            break;
                        default:
                            //only cache static html pages
                            var parts = url.split('.');
                            if (parts.length > 1 && parts[parts.length - 1] == 'html') {
                                //cache partials to improve performance
                                settings.partialCache[url] = xmlhttp.responseText;
                            }
                    }
                    console.log('ajaxRequest======== response', xmlhttp.status, xmlhttp.responseText);
...
miniSPA.refresh = function(node, scope) {
    ...
    if (node.nodeType != 3) {
        //traverse child nodes, Node.TEXT_NODE == 3
